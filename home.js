...
let currentFilter = "All Hometowns";
...
const uniqueHometowns = [...new Set(brothersData.map(brother => brother.hometown))];
uniqueHometowns.unshift("All Hometowns");
...
filterButton.addEventListener("click", () => {
    ...
    let index = uniqueHometowns.indexOf(currentFilter);
    currentFilter = uniqueHometowns[(index + 1) % uniqueHometowns.length];
    ...
    images.forEach((pic) => {
        ...
        if (currentFilter === "All Hometowns" || brother.hometown === currentFilter) {
            question.value = "What is this person's hometown?";
            hometown.value = brother.hometown;
            image.value = imageFolder + pic;
            answer.value = hometown.value;
            viewlist(); // This function should create and display a card for the current brother
        }
    });
  });
...
function viewlist() {
    ...
    displayAnswer.innerText = hometown.value;
