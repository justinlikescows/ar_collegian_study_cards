...
let currentFilter = "All Industries";
const uniqueAreasOfInterests = [
    "Communications",
    "Advocacy",
    "Consulting",
    "Asset Management",
...
filterButton.addEventListener("click", () => {
    ...
    let index = uniqueAreasOfInterests.indexOf(currentFilter);
    currentFilter = uniqueAreasOfInterests[(index + 1) % uniqueAreasOfInterests.length];
    ...
    // Refresh the display
    images.forEach((pic) => {
      question.value = "What is this person's area of interest?";
    const brother = brothersData.find(brother => brother.image === pic);
    industry.value = brother.industry_interests[0];
    if (brother.industry_interests.length > 1 && brother.industry_interests[1] === currentFilter) {
        industry.value = brother.industry_interests[1];
    }
    if (brother.industry_interests.length > 2 && brother.industry_interests[2] === currentFilter) {
        industry.value = brother.industry_interests[2];
      }
      image.value = imageFolder + pic;
      answer.value = industry.value;
    //   year.value = brothersData.find(brother => brother.image === pic).year;
      console.log(currentFilter);
      if (currentFilter === "All Industries" || industry.value === currentFilter) {
        viewlist();
      }
    });
  });
...
window.onload = () => {
    images.forEach((pic) => {
      question.value = "What is this person's area of interest?";
      answer.value =  brothersData.find(brother => brother.image === pic).industry_interests;
      image.value = imageFolder + pic;
